...
# Extract keywords from the documention (html) of GetDP
# that are located in the table "Syntax Index":
# http://getdp.info/doc/texinfo/getdp.html#Syntax-index
# http://getdp.info/doc/texinfo/getdp.html
# The script writes N JSON files that should then be copy/paste 
# into the language file of the GetDP extension.
# The split in different files is due to the (too) high number of 
# keywords, leading to a bug (on my computer) 
...
# <tr><td></td><td valign="top"><a href="#ANCHOR_TO_COMMAND"><code>KEYWORD + OPTIONS</code></a>:</td><td>&nbsp;</td><td valign="top"><a href="#ANCHOR_TO_TYPE">TYPE_OF_KEYWORD</a></td></tr>
    ...
    if(section == "Command-line options"):
      continue
    if(section == "Operators" or section=="Operator types" or section == "General commands" or section == "Comments" or section == "Running GetDP" or section == "Evaluation order"):
      continue
